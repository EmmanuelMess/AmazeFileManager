...
public class MainFragment extends android.support.v4.app.Fragment implements BottomBarButtonPath {
    ...
    private LayoutElementsManager layoutElementsManager;
    ...
    public void onActivityCreated(final Bundle savedInstanceState) {
        ...
        layoutElementsManager = new LayoutElementsManager();
    ...
    public void switchView() {
        createViews(layoutElementsManager.getLayoutElementsClone(), false, CURRENT_PATH,
                openMode, results, checkPathIsGrid(CURRENT_PATH));
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (listView != null) {
            ...
            outState.putParcelable(KEY_LAYOUT_ELEMENTS, layoutElementsManager);
    ...
    void onSavedInstanceState(final Bundle savedInstanceState) {
        ...
        if (cur != null) {
            ...
            layoutElementsManager = savedInstanceState.getParcelable(KEY_LAYOUT_ELEMENTS);
            CURRENT_PATH = cur;
            ...
            createViews(layoutElementsManager.getLayoutElementsClone(), true, (CURRENT_PATH),
                    openMode, results, !IS_LIST);
            ...
            switch (item.getItemId()) {
                ...
                case R.id.share:
                    ...
                    if (arrayList.size() > 100)
                        ...
                        switch (layoutElementsManager.getLayoutElement(0).getMode()) {
                            case DROPBOX:
                            case BOX:
                            case GDRIVE:
                            case ONEDRIVE:
                                FileUtils.shareCloudFile(layoutElementsManager.getLayoutElement(0).getDesc(),
                                        layoutElementsManager.getLayoutElement(0).getMode(), getContext());
                                break;
                            default:
    ...
    public void createViews(ArrayList<LayoutElementParcelable> bitmap, boolean back, String path,
        ...
        if (bitmap != null && isAdded()) {
            ...
            layoutElementsManager.putLayoutElements(bitmap);
            if (grid && IS_LIST)
                switchToGrid();
            ...
            if (adapter == null) {
                ...
                adapter.setItems(layoutElementsManager.getLayoutElementsClone());
            }
            stopAnims = true;
    ...
    void fixIcons(boolean forceReload) {
        if (layoutElementsManager.getLayoutElementsClone() == null) return;
        Drawable iconDrawable;

        for (LayoutElementParcelable layoutElement : layoutElementsManager.getLayoutElementsClone()) {
    ...
    private LayoutElementParcelable addTo(HybridFileParcelable mFile) {
        ...
        if (!dataUtils.isFileHidden(mFile.getPath())) {
            if (mFile.isDirectory()) {
                ...
                layoutElementsManager.addLayoutElement(layoutElement);
                folder_count++;
    ...
    public void addSearchResult(HybridFileParcelable a, String query) {
        if (listView != null) {
            ...
            if (!results) {
                layoutElementsManager.clearLayoutElement();
                file_count = 0;
                folder_count = 0;
            }
    ...
    public void onSearchCompleted(final String query) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                Collections.sort(layoutElementsManager.getLayoutElementsClone(),
            ...
            public void onPostExecute(Void c) {
                createViews(layoutElementsManager.getLayoutElementsClone(), true, (CURRENT_PATH), openMode, true, !IS_LIST);// TODO: 7/7/2017 this is really inneffient, use RecycleAdapter's createHeaders()
                getMainActivity().getAppbar().getBottomBar().setPathText("");
    ...
    public ArrayList<LayoutElementParcelable> getLayoutElementsClone() {
        return layoutElementsManager.getLayoutElementsClone();
    }
        ...
        public void onEvent(int event, String path) {
            ...
            switch (event) {
                ...
                case MOVED_TO:
                    ...
                    for (String pathAdded : pathsAdded) {
                        ...
                        layoutElementsManager.addLayoutElement(fileCreated.generateLayoutElement(MainFragment.this, utilsProvider));
                    }
                ...
                case MOVED_FROM:
                    pathsRemoved.add(path);
                    for (int i = 0; i < layoutElementsManager.getLayoutElementSize(); i++) {
                        File currentFile = new File(layoutElementsManager.getLayoutElement(i).getDesc());

                        for (String pathRemoved : pathsRemoved) {
                            if (currentFile.getName().equals(pathRemoved)) {
                                layoutElementsManager.removeLayoutElement(i);
                                break;
                            }
                        }
                    }
            ...
            getActivity().runOnUiThread(() -> {
                if (listView.getVisibility() == View.VISIBLE) {
                    ArrayList<LayoutElementParcelable> elements = layoutElementsManager.getLayoutElementsClone();

                    if (layoutElementsManager.getLayoutElementSize() == 0) {
